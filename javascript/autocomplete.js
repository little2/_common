...
*/
    ...
    */
    ...
    {    	
		...
		// 不區分大小寫
		var new_str=value.substr(0, (pre_value.length));				
		if (value.length>0 && pre_value>0 && (new_str.toUpperCase() == pre_value.toUpperCase()) )
        { 
        	AutoComplete_ShowDropdown2(id);
        } 
		else
		{
			CallXajax('show_'+id+'_suggest_list',__AutoComplete[id]['element']);
		}
		
	}	
	
	function AutoComplete_ShowDropdown2(id)
	{
        ...
        {       
       		content=__AutoComplete[id]['data'][i];
   			
           // if (__AutoComplete[id]['data'][i].substr(0, value.length) == value) 
           // 不區分大小寫
           var content_UW = content.toUpperCase();
           var value_UW = value.toUpperCase();
           
           if( (content_UW.indexOf(value_UW))>=0)
            {
